package com.lekha.stage
{
	...
	public class ExchangeCards extends Game implements IStage
	{
			...
			for each( object in _cardsOnTableOldPositions ) {
				...
				for(var j:int=0; j<cards.length; j++) {				
					if ( beforeChair.player.id == _myChair.player.id ) {
						Logger.log("this person", chair.player.name);
						Logger.log("gave me", beforeChair.player.name);
						Logger.log("this card", _recievedCards.cards[j]);
